...
import { Newspaper, RefreshCw, ExternalLink, Calendar, Link as LinkIcon } from 'lucide-react';
          ...
          <div className="space-y-4">
            ...
            {sources.length > 0 && (
              <div className="mt-4 pt-4 border-t border-slate-200/50">
                <p className="text-[10px] font-black text-emerald-600 mb-2 uppercase">روابط المصادر الرسمية:</p>
                <div className="flex flex-wrap gap-2">
                   {sources.map((source: any, idx: number) => (
                      <a 
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-600 hover:text-emerald-600 hover:border-emerald-200 transition-colors"
                      >
                         <LinkIcon size={10} />
                         {source.web?.title?.substring(0, 30) || 'رابط المصدر'}...
                      </a>
                   ))}
                </div>
              </div>
